...
<form on:submit|preventDefault={handleSubmit}>
  <fieldset class="flex gap-4 mt-8">
    {#each options as option}
      <label class="chip text-base font-semibold {selected.includes(option) ? "variant-ghost opacity-50" : "variant-filled"}">
        <input
          class="hidden"
          type="checkbox"
          name="flavours"
...
<style>
  .answer-container {
    ...
    background-image: linear-gradient(180deg, transparent 0, transparent var(--line-1), white var(--line-1), white var(--p1), transparent var(--p1), transparent var(--line-2), white var(--line-2), white var(--p2), transparent var(--p2), transparent var(--line-3), white var(--line-3), white var(--p3), transparent var(--p3));
  }
</style>
